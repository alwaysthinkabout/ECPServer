...
/************************公司待审核信息列表******************************/
function enterpriseInfoAuditList(){
	grid= $('#enterpriseInfoAuditListTable').jqGrid({ 
		  //caption: "网点信息",
		  datatype:'local',  
		  //altRows: true,
		  colNames:['营业执照号','公司名称','操作时间','待审核条数',''],
		  shrinkToFit:false,
		  height:400,
		  rowNum:10,
	      rowList: [10, 20, 30],
...
function getEnterpriseInfoAuditList(){
	enterpriseInfoAuditList();
	cVe.startReqByMsgHandle(cVeName,'','','reqGetEnterpriseInfoAuditList','resGetEnterpriseInfoAuditList','ECP.handle.platform.OrgAuditHandle.handleMsg');

}

function reqGetEnterpriseInfoAuditList(){
	var req = {};	
	req["op"] = "auditWaitingList";
	cVe.setConn(cServerUri,"POST", true, JSON.stringify(req));
}

function resGetEnterpriseInfoAuditList(){
...
function displayButtons(cellValue,options,rowObject){
	...
	var audit = '<button class=" btn btn-primary btn-sm"  onclick="audit('+org_id+')">审核</button>';
	return audit;
}

function audit(org_id){
	window.location.href="enterpriseAuditDetailPage.jsp?org_id="+org_id;
	cVeUti.Cookie.setCookie("pageUrl3","enterpriseAuditDetailPage.jsp?org_id="+org_id);
	cVeUti.Cookie.setCookie("pageName3","企业信息审核详情");
}
