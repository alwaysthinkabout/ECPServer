...
/************************公司待审核信息列表******************************/
function storeInfoAuditedList(){
	grid= $('#storeInfoAuditListTable').jqGrid({ 
		  //caption: "网点信息",
		  datatype:'local',  
		  //altRows: true,
		  colNames:['店面名称','联系人姓名','审核时间','审核人','审核结果',''],
		  shrinkToFit:false,
		  height:400,
		  rowNum:10,
	      rowList: [10, 20, 30],
		  colModel:[  
		    ...
		    {id:'audit_user_name',name:'audit_user_name', width:'100%',align:'center'},
...
function getStoreInfoAuditedList(){
	storeInfoAuditedList();
	cVe.startReqByMsgHandle(cVeName,'','','reqGetStoreInfoAuditedList','resGetStoreInfoAuditedList','ECP.handle.platform.OrgAuditHandle.handleMsg');

}

function reqGetStoreInfoAuditedList(){
	var req = {};	
	req["op"] = "getStoreInfoAuditedList";
	cVe.setConn(cServerUri,"POST", true, JSON.stringify(req));
}

function resGetStoreInfoAuditedList(){
...
function displayButtons(cellValue,options,rowObject){
	...
	var audit = '<button class=" btn btn-primary btn-sm"  onclick="audit('+audit_id+')">查看</button>';
	return audit;
}

function audit(audit_id){
	window.location.href="storeInfoAuditedDetailPage.jsp?audit_id="+audit_id;
}
    ...
    setTimeout(function(){//因为是即时查询，需要用setTimeout进行延迟，让值写入到input内，再读取  
      ...
      /*  alert("storeName:"+(storeName=="")+"  oper_type:"+(oper_type==""));
    	...
    	for(var i=1;i<rowsLength;i++){//按表的行数进行循环，本例第一行是标题，所以i=1，从第二行开始筛选（从0数起）  
            ...
            var searchAuditResultText = storeId.rows[i].cells[searchAuditResult].innerHTML;//取得table行，列的值 
          /*  alert(searchDateText);*/
            ...
            if(audit_result!=""){
            	condition=condition&&searchAuditResultText==audit_result;//用match函数进行筛选，如果input的值，即变量 key的值为空，返回的是ture， 
            }
            if(condition){ 
