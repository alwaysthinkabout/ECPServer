/*
document.write('<script type="text/javscript" src="js/jquery/jquery.min.js"></script>');
	...
	var init = function() {
			var jqObj = {
				...
				onSelectAll: function(aRowids, status) {
					console.log("onSelectAll:start");
					var p = (showData.page - 1) * (options.rows);
					for (var i = 1; i <= aRowids.length; i++) {
						checks[p + i] = status;
					}
					console.log("onSelectAll:end");
				},
				loadComplete: function() {
					console.log("loadComplete:start");
					//先删再插入数据，保证无重复
				...
				localReader: { //  
					...
					total: function(obj) {
						console.log("from total"+showData);
						return showData.total;
					},
					records: function(obj) {
						console.log("from records"+showData);
			...
			jqObj.colModel = [];
			for (var i = 0; i < options.colNames.length/*colFields.length*/; i++) {
				var m = {};
				m.name = i + '';
				//m.name = options.colFields[i];//i + '';
				m.align = "center";
				//m.index = options.colFields[i];
				//注入formatter
