 ...
 function JsVeMsgDispatcher()
 {
  ...
  this.setConn=function(serverUri,method,isAsyn,sendData)
   {//alert("从消息处理函数 进入ve的setConn");
 	...
 	if(scheduledLogout) scheduledLogout();
...
function Buffer()
{
    ...
    //@result {boolean}
    //weiwenjie由delete改成delete1
    this.delete1 = function(start,num){
        if(start<0)
            return false;
        if(buffer.length===0)
            return true;
...
function Grid2(selector, buffer, options) {
	...
	var init = function() {
			...
			jqObj.colModel = [];
			for (var i = 0; i < options.colFields.length; i++) {
				var m = {};
				m.name = options.colFields[i];
