...
//获取默认省份的下辖市
var deaultProvince = document.getElementById('province').value;
var dafaultCity = document.getElementById('city');
for(var i=0;i<cityArr.length;i++)
	{
	    if(deaultProvince == cityArr[i][0])
    	{
	    	for(var j = 1; j < cityArr[i].length; j++)
    		{
		    	var childOption = document.createElement('option');  
	            //ie不支持option对象的value,所以加childOption.text  
	            childOption.value = childOption.text = cityArr[i][j];  
	            dafaultCity.options.add(childOption);
    		}
            break;
    	}
	}
...
//    var provinceOption = document.createElement('option');  
//    provinceOption.value = provinceOption.text = provinceArr[key];  
//    province1.options.add(provinceOption);  
//} 
//生成市县、区市'  
